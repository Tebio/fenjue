...
## 背景
任务B 已验证规则「昨涨停+今开≥5% + 今开幅度6~7% + 首30分钟量能比<1.0」（挂+7%限价，T+1收盘出）。
现需每日影子记账基建：盘后登记当日信号+模拟成交，次日回填收益，向前积累前向证据。

## 交付物
新文件 engine/zhaban_daily.py，完整可运行。

## 逻辑
1. 读 data/zhaban_shadow.jsonl（不存在则空）：每行一个登记 {date, code, name?, fill_px(或null=未成交), t1_close_ret(或null=待回填)}
2. 回填：对所有 t1_close_ret 为 null 的登记，若其 date 的下一交易日已有数据（big_kcache 前复权 close），
   填 t1_close_ret = 下一交易日close/fill_px - 1 - 0.0015（fill_px=null 的登记直接标 "unfilled" 不再追踪）
3. 登记当日信号：对 data/m60_cache/*.json 全量扫描「当日」信号（若当日无 m60 数据=非交易日/数据未就绪，安静退出码0）
   - 信号定义与成交模拟：完全复用附后源码的判定（昨涨停≥+9.8%、今开≥+5%、今开幅度6~7%、
     首30分钟量能比<1.0、挂+7%限价 bar.open/bar.low 成交规则）
   - 幂等：同日同 code 重复运行不产生重复登记
4. 输出 stdout 紧凑汇报：新登记N只（名单+成交/未成交）、回填M笔（均笔%/累计胜率/累计均笔）
5. 汇总写 data/zhaban_shadow_summary.json：{总登记, 已回填, 成交率, 均笔%, 胜率%, 最近10笔}

## 约束
- stdlib only；名称映射用 data/industry_map.json
- 回填用 big_kcache 前复权价 vs fill_px（m60不复权）存在分红跨度口径差——接受并在文件头注记（与 B5 影子同口径）
- 硬断言：回填日必须严格晚于登记日

## 附：engine/zhaban_features.py 源码（复用其信号/成交判定）
```python
#!/usr/bin/env python3
