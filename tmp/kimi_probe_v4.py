...
if __name__ == "__main__":
    ...
    cmd = (f'cd /d "{TMP}" && node driver.js "{TMP}\\reqs.json" "{TMP}\\out2.json" "{MCP_DIR}"')
    ...
    p2 = subprocess.run(SSH + [
        f'powershell -NoProfile -Command "$f=\'{TMP}\\out2.json\'; if(Test-Path $f){{$s=Get-Content $f -Raw; $s.Substring(0,[Math]::Min(6000,$s.Length))}} else {{\'NOFILE\'}}"'
