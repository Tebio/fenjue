...
def main() -> int:
    ...
    # ── Daily Delta: compare today vs yesterday detailed scores ──────
    # Uses research/data/scores/ snapshot files generated by ScoringEngine.
    # Gracefully handles missing data (e.g. first run, holiday).
    try:
        from engine.daily_delta import DailyDelta
        delta = DailyDelta()
        # Determine the reference date: prefer snapshot date, then today
        ref_date = args.snapshot_date if args.snapshot_date else None
        print()
        print(delta.generate(delta_date=ref_date))
    except Exception:
        pass  # daily delta is optional — never block the main report
