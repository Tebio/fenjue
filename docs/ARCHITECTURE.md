...
m60(分钟线)      ─┘                     ├→ capacity_sim（成簇K/深跌选票/出场规则）
                                        ...
                                        └→ claims_shadow 影子盘（前向 L5）
                                                                 ↓
   claims_state.json ──────────────────────────────────────────→ 推送层（最后一英里已接线）：
   （周审计状态机）            推送=扫描+确认双闸 · 成簇门（零星日毒不出手）· DECAYING横幅
                              （reversal_daily/ops_briefing/dashboard 同源读状态机）
                              + 恐慌复活门（跌停潮≥50 宽篮子）+ 摇篮成簇（年1-3次）
```

**反馈闭环**：影子盘回填 → 周审计滚动边际 → kill 线触发降级 → **推送读 claims_state 自动降级/静默
（2026-09-19 接线：此前闭环断在最后一英里）** → 恢复条件达标再复活。
...
| **dsr_recount / triples_exhaustive / monday_combo / allocation_sim / winner_anatomy / demon_anatomy** | ⚠️ 今晚新建，同样直接 import law_pipeline（fwd 口径已与 fjcore.forward 逐行比对一致） | 加入收编队列，下轮换 fjcore |
